...
export default function PromptStudio({
  ...
  const handleDelete = async () => {
    ...
    try {
      await authFetch(`${API_BASE_URL}/prompts/${activePromptId}`, {
        method: "DELETE",
      });
        ...
        <div className="mt-6 fade-in">
          <div className="flex gap-6 border-b border-white/5 pb-4">
            <div>
              ...
              <div className="flex items-center border-b border-border-color w-[90px] py-0.5 transition-colors duration-300 focus-within:border-brand">
                <button
                  className="bg-transparent text-[#7a8294] border-none cursor-pointer text-[1.1rem] px-2 select-none hover:text-white"
                  onClick={() => setModelCentric(Math.max(0, modelCentric - 1))}
                >
                  −
                </button>
                <input
                ...
                <button
                  className="bg-transparent text-[#7a8294] border-none cursor-pointer text-[1.1rem] px-2 select-none hover:text-white"
                  onClick={() => setModelCentric(modelCentric + 1)}
                >
                  +
                </button>
              </div>
            ...
            <div>
              ...
              <div className="flex items-center border-b border-border-color w-[90px] py-0.5 transition-colors duration-300 focus-within:border-brand">
                <button
                  className="bg-transparent text-[#7a8294] border-none cursor-pointer text-[1.1rem] px-2 select-none hover:text-white"
                  onClick={() => setEnhancedProduct(Math.max(0, enhancedProduct - 1))}
                >
                  −
                </button>
                <input
                  ...
                  onChange={(e) =>
                    setEnhancedProduct(Number(e.target.value) || 0)
                  }
                />
                <button
                  className="bg-transparent text-[#7a8294] border-none cursor-pointer text-[1.1rem] px-2 select-none hover:text-white"
                  onClick={() => setEnhancedProduct(enhancedProduct + 1)}
                >
                  +
                </button>
              </div>
          ...
          <div className="flex gap-6 mt-6 pb-0 border-none">
            <div className="flex-1">
              ...
              >
                <option value="" disabled className="bg-bg-dark text-white">
                  Custom
                </option>
                {prompts.map((p) => (
                  <option
          ...
          <div className="flex gap-4 items-center mt-6 pt-4 border-t border-white/5">
            <button
              className="bg-[#2a2e38] text-gray-300 border border-[#3b404d] px-5 py-2 rounded-full font-medium text-[0.85rem] cursor-pointer transition-all duration-200 hover:bg-[#373c47] hover:text-white hover:border-[#4a5060] hover:-translate-y-[1px]"
              onClick={handleSave}
            >
              Save changes
            </button>
            <button
              className="bg-transparent text-text-secondary border-none text-[0.85rem] cursor-pointer transition-colors duration-200 hover:text-white"
              onClick={handleAdd}
            >
              + New prompt
            </button>
